...
def aplica_Sobel(imatges):
    ...
    for i, imatge in enumerate(imatges):
        ...
        sobel_imatge = cv2.Sobel(imatge, cv2.CV_32F, orden_x, orden_y, ksize=tamany_kernel)
        sobel_imatge = ((sobel_imatge / np.max(np.abs(sobel_imatge)) + 1) * 127.5).astype(np.uint8)
...
def main():
    ...
    sobel_imatges, ordenes_kernels = aplica_Sobel(imatges)
